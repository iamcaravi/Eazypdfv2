...
(function(){
  ...
  const entranceTargets = [header, heroBadge, ...heroLines, heroTagline, heroCta, heroArt, sectionHead, ...cards].filter(Boolean);
  if(MOTION.reduced){
    gsap.set(entranceTargets, {autoAlpha:1, y:0, scale:1});
    return;
  }
  // Unconditional safety net: force full visibility after a fixed delay no
  // matter what GSAP itself reports. Confirmed in the field that a heavily
  // staggered .from() tween (40 tool cards) can report onComplete via a
  // lag-smoothing time-jump on a throttled/backgrounded tab WITHOUT ever
  // actually rendering the final visible state for every target - so this
  // must not be gated on (or cancelled by) the timeline's own onComplete.
  // Real page content can never be allowed to stay invisible because an
  // entrance animation's own "done" signal turned out to be untrustworthy.
  setTimeout(()=>{
  ...
  if(cards.length) tl.from(cards, {autoAlpha:0, y:14, duration:MOTION.normal, stagger:{amount:MOTION.emphasis, from:"start"}}, "-=0.15");
})();
...
(function(){
  ...
  // Card spotlight: one delegated listener for the whole grid (not one
  // per card) sets --mx/--my (percentages, cheap to compute) only on
  // whichever card the pointer is actually over - the CSS above does
  // the rest (opacity gated on :hover already, so idle cards never pay
  // for a gradient repaint).
  const toolsGrid = document.getElementById("toolsGrid");
  if(toolsGrid && !MOTION.reduced){
    toolsGrid.addEventListener("pointermove", e=>{
